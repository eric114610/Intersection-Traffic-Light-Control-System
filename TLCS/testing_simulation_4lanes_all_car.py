...
# envr w/ bus & truck, seed 1000 <- 25969(21912 for accel = 2.6)
...
class Simulation:
    ...
    def _choose_action(self, state, current_flow, old_flow, old_action):
        ...
        NS_turn_queue= traci.lane.getLastStepHaltingNumber("N2TL_3") + traci.lane.getLastStepHaltingNumber("S2TL_3")
        WE_turn_queue= traci.lane.getLastStepHaltingNumber("W2TL_3") + traci.lane.getLastStepHaltingNumber("E2TL_3")
        ...
        if current_flow < 3 and old_flow < 3:
            if old_action == 1 or old_action == 3:
                if current_flow < 1 and self._get_queue_length() < 5 and self._get_queue_length() > 0:
                    if old_action == 1:
                ...
                if self._get_queue_length() < 5 and self._get_queue_length() > 0:
                    if old_action == 0:
                        if NS_turn_queue > WE_straight_queue + WE_turn_queue:
                            return 1
                        else:
                            if WE_straight_queue > WE_turn_queue:
                                return 2
                        ...
                        if WE_turn_queue > NS_straight_queue + NS_turn_queue:
                            return 3
                        else:
                            if NS_straight_queue > NS_turn_queue:
                                return 0
